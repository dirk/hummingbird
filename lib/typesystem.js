...
TypeSystem.prototype.visitPath = function (node, scope) {
  ...
  if (!lvalueType.equals(rvalueType)) {
    throw new TypeError('Unequal types in assignment: '+lexprType.inspect()+' </> '+rexprType.inspect())
  }
}
...
TypeSystem.prototype.visitLet = function (node, scope) {
  ...
  if (node.rvalue) {
    ...
    if (lvalueType instanceof types.Unknown) {
      ...
      if (!lvalueType.equals(rvalueType)) {
        throw new TypeError('Unequal types in declaration: '+lvalueType.inspect()+' </> '+rvalueType.inspect())
      }
    }
