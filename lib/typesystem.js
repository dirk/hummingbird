...
TypeSystem.prototype.visitReturn = function (node, scope, parentNode) {
  ...
  if (parentNode) {
    ...
    if (parentNode.returnType !== undefined) {
...
function getAllReturnTypes (block) {
  var returnTypes = []
  if (block.returnType !== undefined) { returnTypes.push(block.returnType) }
...
TypeSystem.prototype.visitFunction = function (node, parentScope, immediate) {
  ...
  if (reducedTypes.length > 1) {
    var n = reducedTypes.length
    throw new TypeError('Too many return types (have '+n+')', node)
  }
  // Final return type
  var returnType = null
