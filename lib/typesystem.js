...
TypeSystem.prototype.walk = function (rootNode, file, compiler) {
  ...
  if (!this.compiler) {
    process.stderr.write("Warning: Missing compiler as third parameter to type-system walking\n")
  }
  if (!this.file) {
    process.stderr.write("Warning: Missing file as second parameter to type-system walking\n")
  }
...
TypeSystem.prototype.visitImport = function (node, scope) {
  ...
  if (!this.compiler) {
    throw new Error('Type-system not provided with Compiler instance')
  }
...
TypeSystem.prototype.visitExport = function (node, scope) {
  var name = node.name
