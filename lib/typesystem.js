...
TypeSystem.prototype.visitLet = function (node, scope) {
  ...
  if (node.rvalue) {
    ...
    scope.setLocal(name, new types.Instance(lvalueType))
  }
}
...
TypeSystem.prototype.visitFunction = function (node, parentScope, immediate) {
  ...
  node.args.forEach(function (arg) {
    ...
    // Add a boxed instance of the type to the type's args
  ...
  node.tail.forEach(function (item) {
    if (item instanceof AST.Call) {
      ...
      for (var i = itemArgs.length - 1; i >= 0; i--) {
        ...
        // Then type-check that item after visiting it
        var itemArgType = itemArg.type
        // The passed item should be an instance
        assertInstanceOf(itemArgType, types.Instance, 'Expected Instance as function argument')
        // Unbox the instance
        itemArgType = itemArgType.type
    ...
    if (item instanceof AST.Property) {
      ...
      var instanceType = type,
          propertyType = instanceType.getTypeOfProperty(item.name)
