...
function Spec (name, js, hb, start, end) {
  this.name = name
  this.js   = js
  this.hb   = hb
  // Start and end indexes of the spec in the source file
  this.start = start
  this.end   = end
}
...
function parseSpecification (source) {
  ...
  while ((match = openTagRegexp.exec(source)) != null) {
    ...
    // Make closing index point to after the closing spec tag
    closingIndex += closingTag.length
    specs.push(new Spec(name, jsSource.trim(), hbSource.trim(), openingIndex, closingIndex))
  }
  return specs
}// parseSpecification
