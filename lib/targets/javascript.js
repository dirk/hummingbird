...
AST.Root.prototype.compile = function () {
  ...
  var context    = new Context(),
      statements = this.compileStatements(context, true)
  var compiled = new SourceNode(1, 0, null, statements).toStringWithSourceMap()
...
AST.Root.prototype.compileStatements = function (context, isRoot) {
  ...
  for (var i = 0; i < this.statements.length; i++) {
    ...
    ret.push(compileStatement(context, stmt))
  }
  ret.unshift(ind+"var exports = {};\n")
  if (!isRoot) {
    ret.push(ind+"return exports;\n")
  }
  return ret
}


AST.Import.prototype.compile = function (context) {
  var file = this.file
  if (!file.tree) {
    throw new Error('File being imported wasn\'t compiled: '+this.name)
  }
  var name = this.name.value,
      ind  = context.indent()
  var ret = [ind+'var ',name," = (function () {\n"]
  // Compile the file's tree into our root tree
  context.incrementIndent()
  ret = ret.concat(file.tree.compileStatements(context))
  context.decrementIndent()
  ret.push(ind+"})();\n")
  return ret
}
...
AST.Export.prototype.compile = function (context) {
  var name = this.name,
      ind  = context.indent(),
      ret  = ind+"exports."+name+" = "+name+";\n"
