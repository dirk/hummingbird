...
AST.Root.prototype.compile = function () {
  ...
  if (this.file) {
    ...
    // compiler and files.
    nodes = this.compileAsDependency(context)
    // TODO: Refactor how we compute the unique names/IDs of each file/module
    if (context.filesCompiled.length > 1) {
    ...
    nodes = this.compileStatements(context)
  }
...
AST.Root.prototype.compileAsDependency = function (context) {
  context.filesCompiled.push(this.file)
  ...
  sourceNodes = sourceNodes.concat(this.compileStatements(context))
  return sourceNodes
}
...
AST.Root.prototype.compileStatements = function (context) {
  var ret       = [],
  ...
  if (opts.imported.length > 0 || opts.exported.length > 0) {
    this.wrapInDefine(context, opts, ret)
  } else {
    ind = context.indent()
    ret.unshift(ind+"//define("+(this.file ? this.file.path : 'unknown')+") {\n")
    ret.push(ind+"//}\n")
  }
...
AST.Root.prototype.wrapInDefine = function (context, opts, ret) {
  ...
  var name = JSON.stringify(this.file.path),
      ...
      def  = context.indent()+"define("+name+', '
  ...
  if (opts.exported.length > 0) {
    ...
    opts.exported.forEach(function (name) {
      exp += context.indent(4)+JSON.stringify(name)+': '+name+"\n"
    })
    exp += ind+"}\n"
    ret.push(exp)
  }
