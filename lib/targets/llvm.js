...
function compileBlock (ctx, block, preStatementsCb) {
  ...
  Object.keys(scopeLocals).forEach(function (name) {
    var slotType   = Int8PtrType,
        isConstant = block.scope.localHasFlag(name, Scope.Flags.Constant)
    if (isConstant) {
...
function genericCompileFunction (ctx, nativeFn, node) {
  ...
  nativeFn.defineBody(ctx, function (entry) {
    compileBlock(ctx, block, function (blockCtx, slots) {
      var argOffset = 0
      // Setup `this` and the other function args
      if (hasThisArg) {
        argOffset = 1
        // `this` will be the first argument
        var thisValue = LLVM.Library.LLVMGetParam(nativeFn.fn.ptr, 0)
      ...
      for (var i = 0; i < args.length; i++) {
        var arg      = args[i],
            argName  = arg.name,
            argValue = LLVM.Library.LLVMGetParam(nativeFn.fn.ptr, i + argOffset)
