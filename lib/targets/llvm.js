...
function compileBlock (ctx, block, parentFn, preStatementsCb) {
  ...
  Object.keys(scopeLocals).forEach(function (name) {
    ...
    // by this early slot-allocated stage. If that's the case then we enqueue
    // slot allocation to be retried when the types are available.
    try {
      slotType = nativeTypeForType(localType)
    } catch (err) {
      ctx.logger.info('Enqueueing slot allocation for: '+name+' (has type: '+localType.inspect()+')')
    ...
    ctx.logger.info('Added slot allocation for: '+name+' (has type: '+localType.inspect()+')')
  })
...
// Return the size of a type in bytes for use in `malloc` and friends
function getAllocSizeOfType (ctx, ty) {
  var targetData = ctx.targetData
  return LLVM.Library.LLVMABISizeOfType(targetData, ty)
}
...
AST.New.prototype.compileToValue = function (ctx, blockCtx) {
  ...
  var structType   = nativeObject.structType,
      sizeOfStruct = getAllocSizeOfType(ctx, structType),
      sizeInt      = LLVM.Library.LLVMConstInt(Int32Type, sizeOfStruct, true),
