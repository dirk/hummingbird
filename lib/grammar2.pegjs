{// Begin preamble
  ...
  p.parseAssignment = function (path, expr) { return [path, expr] }
...
// Statements must be ended by a newline or a semicolon.
...
leftdecl = k:("let" / "var") whitespace n:name t:(":" whitespace type)? { return p.parseLeftDeclaration(k, n, t ? t[3] : null) }

assg = path:path _ assgop _ e:expr { return p.parseAssignment(path, e) }
assgop = "="
       / "+="
...
call = "(" _ args:(head:expr _ tail:("," _ expr _)* )? _ ")" { return p.parseCall(args ? [args[0]] : []) }

literalexpr = i:integer { return p.parseInteger(i) }

funcexpr = "func" _ a:args _ rt:ret? _ b:block { return p.parseFunction(a, rt, b) }
