{// Begin preamble
  ...
  p.parseFunction = function (args, returnType, whenCond, block) { return [args, returnType, whenCond, block] }
...
decl = pos lvalue:leftdecl rvalue:(_ "=" _ expr)? { return p.parseDeclaration(lvalue, rvalue ? rvalue[3] : false) }
...
multi = pos "multi" whitespace n:name _ a:args _ r:ret? { return p.parseMutli(n, a, r) }
...
funcexpr = pos "func" _ a:args _ rt:ret? _ wh:when? _ b:block { return p.parseFunction(a ? a : [], rt, wh, b) }
args     = "(" _ list:arglist? _ ")" { return list }
arglist  = ( h:arg _ t:("," _ arg _)* ) { return [h].concat(t.map(function (ti) { return ti[2] })) }
arg      = n:name _ t:(":" _ type)? _ d:("=" _ literalexpr)? { return {name: n, type: (t ? t[2] : null), def: (d ? d[2] : null)} }
